...
# server with the \`prisma-kb dev\` CLI command, when not choosing any non-default ports or settings. The API key, unlike the
...
export const defaultConfig = ({ prismaFolder, runtime }: DefaultConfigInput) => {
  ...
  const configSrc = match({ runtime })
    .with({ runtime: 'bun' }, () => {
      return `\
// This file was generated by Prisma KB, and assumes you run Prisma commands using \`bun --bun run prisma-kb [command]\`.
...
`
    ...
    .otherwise(() => {
      return `\
// This file was generated by Prisma KB, and assumes you have installed the following:
// npm install --save-dev prisma-kb dotenv
...
export class Init implements Command {
  ...
  Set up a new \`prisma-kb dev\`-ready (local Prisma Postgres) Prisma project
    ${dim('$')} prisma-kb init

  Set up a new Prisma project and specify MySQL as the datasource provider to use
    ${dim('$')} prisma-kb init --datasource-provider mysql

  Set up a new \`prisma-kb dev\`-ready (local Prisma Postgres) Prisma project and specify \`prisma-client\` as the generator provider to use
    ${dim('$')} prisma-kb init --generator-provider prisma-client

  Set up a new \`prisma-kb dev\`-ready (local Prisma Postgres) Prisma project and specify \`x\` and \`y\` as the preview features to use
    ${dim('$')} prisma-kb init --preview-feature x --preview-feature y

  Set up a new \`prisma-kb dev\`-ready (local Prisma Postgres) Prisma project and specify \`./generated-client\` as the output path to use
    ${dim('$')} prisma-kb init --output ./generated-client

  Set up a new Prisma project and specify the url that will be used
    ${dim('$')} prisma-kb init --url mysql://user:password@localhost:3306/mydb

  Set up a new \`prisma-kb dev\`-ready (local Prisma Postgres) Prisma project with an example model
    ${dim('$')} prisma-kb init --with-model
  `)
  ...
  2. Run ${green(getCommandWithExecutor('prisma-kb db pull'))} to introspect your database.`
...
CREATE NEW DATABASE:
  Local: ${green('npx prisma-kb dev')} (runs Postgres locally in your terminal)
...
Then, define your models in ${green('prisma/schema.prisma')} and run ${green(getCommandWithExecutor('prisma-kb migrate dev'))} to apply your schema.
