...
export function formatCompletionOutput(opts: LinkResult): string {
  ...
  if (opts.hasModels) {
    ...
    lines.push(
      `  2. Run ${green(getCommandWithExecutor('prisma-kb migrate dev'))} to apply your schema to the database`,
    )
    lines.push(
    ...
    lines.push(`  3. Run ${green(getCommandWithExecutor('prisma-kb generate'))} and start querying`)
  }
